...
| G4 | `compiler/`: `olic` in oli-core (design 0022) — lexer, parser, §8 diagnostics, module loader, item collection, signatures, local tables and the checks that need no type graph (capabilities, `E0900`, constants, recursive layouts); reproduces every AST snapshot and the items, signatures and locals of every semantic snapshot; typed bodies and the back end next; fixpoint `stage2 == stage3` | **in progress** |
