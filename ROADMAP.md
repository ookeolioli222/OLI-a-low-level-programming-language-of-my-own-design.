...
| Genesis G3 | `oli1`: oli-core compiler in `machine x64` (asm); steps 0–6d: locals, expressions, strings, control flow, syscalls, procedures, zones, views, raw memory, layouts, refs, fallible results, module constants | **steps 0–6d green (`genesis/test.sh` layer 3); the oli-core subset is complete — next: port `olic` to oli-core (G4)** |
| Genesis G4 | `olic` in oli-core (`compiler/`, design 0022); fixpoint `stage2 == stage3`; passes every fixture in `tests/` | **in progress: lexer, parser, §8 diagnostics, module loader, item collection, signatures and local tables green (`genesis/test.sh` layer 4; all four AST snapshots byte-exact, all fifteen negative parse fixtures exact, the items, signatures and locals of all three semantic snapshots exact); capabilities, scopes, flow, regions and failures enforced (12 of 14 `tests/sema/err` fixtures exact); next: oli1 step 6e (typed places, `rw` field types, `loop`) so the compiler's own source is valid V0, then expression typing and the typed body printer** |
