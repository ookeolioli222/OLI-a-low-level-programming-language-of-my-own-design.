...
| M4 | minimal kernel in Oli-- | **in progress 2026-09-23**: `calls interrupt` (push all, `ref core.x64.InterruptFrame`, `iretq`), `core.x64` layouts, an IDT loaded with `lidt` and a handler reached by `int 3` in `examples/kernel.oli`; next `mem.mmio`, the PIC and a timer, page tables |
