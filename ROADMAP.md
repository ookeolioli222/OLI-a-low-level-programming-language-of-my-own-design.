...
| Genesis G4 | `olic` in oli-core (`compiler/`, design 0022); fixpoint `stage2 == stage3`; passes every fixture in `tests/` | **in progress: the whole front end is green (`genesis/test.sh` layer 4) — all four AST snapshots and all three semantic snapshots byte-exact, all fifteen negative parse fixtures and all fifteen negative semantic fixtures exact, `olic` analyses its own source without a diagnostic and no semantic rule is gated any more; next: OIR, x64 lowering and ELF** |
