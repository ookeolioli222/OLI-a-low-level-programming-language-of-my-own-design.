...
| Genesis G3 | `oli1`: oli-core compiler in `machine x64` (asm); steps 0–6a: locals, expressions, strings, control flow, syscalls, procedures, zones, views, raw memory | **in progress; steps 0–6a green (`genesis/test.sh` layer 3); next: layouts (6b)** |
