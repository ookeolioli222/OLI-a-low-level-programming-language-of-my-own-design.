...
| Genesis G4 | `olic` in oli-core (`compiler/`, design 0022); fixpoint `stage2 == stage3`; passes every fixture in `tests/` | **in progress: lexer, parser, `--show-tokens` and `--show-ast` green (`genesis/test.sh` layer 4; all four AST snapshots byte-exact, all fifteen negative parse fixtures exact); next: semantic analysis against `tests/sema` and `tests/snapshots/*.sema`** |
