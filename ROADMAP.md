...
| Genesis G4 | `olic` in oli-core (`compiler/`, design 0022); fixpoint `stage2 == stage3`; passes every fixture in `tests/` | **in progress: lexer + `--show-tokens` green (`genesis/test.sh` layer 4); next: diag renderer, AST + parser against `tests/snapshots/*.ast`** |
