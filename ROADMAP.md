...
| Genesis G4 | `olic` in oli-core (`compiler/`, design 0022); fixpoint `stage2 == stage3`; passes every fixture in `tests/` | **in progress: lexer, parser, §8 diagnostics, module loader, item collection, signatures and local tables green (`genesis/test.sh` layer 4; all four AST snapshots byte-exact, all fifteen negative parse fixtures exact, the items, signatures and locals of all three semantic snapshots exact); capabilities and `E0900` enforced (3 of 14 `tests/sema/err` fixtures exact); next: typed statements and expressions, regions and flow — the rest of `--show-sema`** |
