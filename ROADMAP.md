...
| Genesis G3 | `oli1`: oli-core compiler in `machine x64` (asm); steps 0–6e: locals, expressions, strings, control flow, syscalls, procedures, zones, views, raw memory, layouts, refs, fallible results, module constants, typed places, `rw` field types, `loop` | **steps 0–6e green (`genesis/test.sh` layer 3); next: step 6f, explicit conversions `T(x)` / `T.wrap(x)`, which stage 3 of `olic` measured as the last gap between oli-core and V0 (93 sites in `compiler/`)** |
| Genesis G4 | `olic` in oli-core (`compiler/`, design 0022); fixpoint `stage2 == stage3`; passes every fixture in `tests/` | **in progress: the whole front end is green (`genesis/test.sh` layer 4) — all four AST snapshots and all three semantic snapshots byte-exact, all fifteen negative parse fixtures and all fourteen negative semantic fixtures exact, `olic` analyses its own source without a diagnostic; next: oli1 step 6f (`T(x)`, `T.wrap(x)` — 93 measured sites), then OIR, x64 lowering and ELF** |
