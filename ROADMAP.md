...
| Genesis G4 | `olic` in oli-core (`compiler/`, design 0022); fixpoint `stage2 == stage3`; passes every fixture in `tests/` | **in progress: lexer, parser, §8 diagnostics, module loader and item collection green (`genesis/test.sh` layer 4; all four AST snapshots byte-exact, all fifteen negative parse fixtures exact, the item section of all three semantic snapshots exact); next: signatures and bodies — the rest of `--show-sema`** |
