...
| `olic` | driver | CLI: `--show-tokens`, `--show-ast`, `--show-sema`, `--check-syntax`, `--check`, `--freestanding`, `--lib` | `--show-tokens` and `--show-ast` exist as `compiler/show_tokens.oli` and `compiler/show_ast.oli` (stdin → stdout, one driver per stage until argv access exists); the rest to be written |
