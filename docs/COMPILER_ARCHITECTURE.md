...
| `oli_sema` | 1b | module loader, `hir` (the semantic graph), name/type resolution, layouts, constant evaluation, flow, regions, capabilities, `--show-sema` printer | **begun: `compiler/load.oli` (imports under `lib/`), `compiler/items.oli` (items, layout/choice layout, constants, signatures) and `compiler/sema.oli` (local tables with type inference) reproduce the items, signatures and locals of every semantic snapshot; typed bodies, regions, capabilities and flow remain** |
