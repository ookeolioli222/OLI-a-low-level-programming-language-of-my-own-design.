...
| `olic` | driver | CLI: `--show-tokens`, `--show-ast`, `--show-sema`, `--show-oir`, `--show-ssa`, `--show-oir=opt`, `--check-syntax`, `--check`, `--freestanding`, `--lib` | the six `--show-*` flags exist as `compiler/show_tokens.oli`, `compiler/show_ast.oli`, `compiler/show_sema.oli`, `compiler/show_oir.oli`, `compiler/show_ssa.oli` and `compiler/show_opt.oli`, `compiler/verify_check.oli` is the verifier's negative test, and `compiler/olic.oli` is the whole pipeline (stdin → a native ELF64 on stdout), one driver per stage until argv access exists; the flags themselves, `--check` and `--lib DIR` wait for command-line access |
| `lib/` | library | `core.oli` (`TrapKind`, `Site`), `core/mem.oli`, `std/os.oli` — written in Oli-- | V0 subset |
| `oli_oir` | 2 | OIR, verifier, passes | **stage 4: `compiler/oir.oli` builds the instruction stream (one instruction per value, statics, a trap site on every operation that can overflow), `compiler/cfg.oli` cuts it into basic blocks with one terminator each and computes predecessors, reverse postorder, immediate dominators and the verifier of `OIR_SPEC.md` §8, `compiler/ssa.oli` is `mem2reg`: places become values and a phi stands where two definitions meet, placed by the iterated dominance frontier; and `compiler/opt.oli` runs the rest of §6 to a fixpoint — constant folding, check elision with a recorded proof for every removal, copy propagation over phis and dead code. `--show-oir`, `--show-ssa` and `--show-oir=opt` print the three forms and nine `tests/snapshots/*.oir`/`*.ssa`/`*.opt` pin them; `compiler/verify_check.oli` breaks the OIR ten ways and the verifier catches each. Common-subexpression elimination is not started; since stage 4 the stream also carries the memory instructions — `addr.of frame`, `raw.load`/`raw.store`, `check.bounds`/`check.range`, `zone.new`/`zone.alloc`/`zone.end` — `mem2reg` keeps the place an address names and places its phis only where the place is live, and the elision pass proves away a bounds or range check on constants** |
| `oli_x64` | 2 | machine lowering, register allocation, encoder | **stage 4: `compiler/x64.oli` lowers the blocks of the SSA form to bytes with every value in its own frame slot — the allocator's worst case, chosen because it needs no liveness analysis. A phi becomes parallel copies on the edges that reach it, through scratch words, so a swap between two phis of one block cannot lose a value. SysV calls and syscalls, the overflow, narrow-width and division checks, and `core.trap`. Register allocation is not started; zones are one anonymous `mmap` with a bump cursor and `munmap` at `end`, view accesses are a bounds check and a sized raw access, and a view result travels in rax:rdx** |
| `oli_elf` | 2 | ELF64 writer | **`compiler/elf.oli`: a static non-PIE executable with one loadable segment (header, statics, code). Section headers and a symbol table are not written, and the file says so rather than writing empty ones** |
