...
| `oli_sema` | 1b | module loader, `hir` (the semantic graph), name/type resolution, layouts, constant evaluation, flow, regions, capabilities, `--show-sema` printer | **begun: `compiler/load.oli` (imports under `lib/`) and `compiler/items.oli` (items, layout/choice layout, constants) reproduce the item section of every semantic snapshot; signatures, bodies, regions, capabilities and flow remain** |
